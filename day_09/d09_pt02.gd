...
func run() -> void:
	...
	file = FileAccess.open("res://day_09/pt02_expanded_output.txt", FileAccess.WRITE)
	file.store_string("".join(map.expanded_map))
	file.close()
	
	map.defrag_keeping_files_together()
	
	file = FileAccess.open("res://day_09/pt02_defrag_output.txt", FileAccess.WRITE)
	file.store_string("".join(map.expanded_map))
	file.close()
